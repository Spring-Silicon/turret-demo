...
def tensor_indexed_memory(encode):
    """Use explicit device indices, not host-list indexing inside graph capture.

    Keep upstream's entire memory computation. The only three AST replacements
    are subscripts of conditioning/unconditioning object lists. Reject source
    changes that invalidate this narrowly pinned adapter.
    """
    import ast
    import inspect
    import textwrap
    ...
    class DeviceIndices(ast.NodeTransformer):
        ...
        def visit_Subscript(self, node):
            ...
            if isinstance(node.slice, ast.Name) and node.slice.id in ('conditioning_objects', 'unconditioning_objects'):
                node.slice.id = 'region_' + node.slice.id
                self.count += 1
            return node
    replace = DeviceIndices()
    replace.visit(tree)
    assert replace.count == 3, 'Upstream memory indexing contract changed'
    for name in ('region_conditioning_objects', 'region_unconditioning_objects'):
...
def install_tracking_regions(torch, model, device_type, *, progress=None):
    ...
    if device_type == 'cuda':
        ...
        def encode_memory(*args, multiplex_state, **kwargs):
            ...
            conditioning = () if conditioning is None else tuple(sorted(conditioning))
            kwargs['conditioning_objects'] = conditioning
            unconditioning = tuple(sorted(multiplex_state.get_all_valid_object_idx() - set(conditioning)))
            for name, indices in (('conditioning', conditioning), ('unconditioning', unconditioning)):
                kwargs['region_' + name + '_objects'] = torch.tensor(indices,
                    dtype=torch.long, device=multiplex_state.mux_matrix.device)
