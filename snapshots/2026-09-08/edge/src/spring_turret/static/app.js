...
function renderDetection(detection) {
  ...
  updatePromptControls();
  const name = detection?.model === "yolo26x" ? "YOLO26x · 80 COCO classes"
    : detection?.model === "sam3.1-tracking" ? "SAM 3.1 Tracking" : "SAM 3.1";
  const labels = { disabled: "Inference not configured", idle: name, loading: `Loading ${name}…`,
    ...
    loading_tracker: "Loading SAM 3.1 Tracking…", waiting_for_camera: "Waiting for camera…" };
  const fresh = isDetectionFresh(detection);
  ...
  detectionMessage.textContent = promptError || detection?.error || (fresh
    ? `${detection.boxes.length} ${detection.boxes.length === 1 ? "box" : "boxes"} · ${fps === null ? "—" : fps.toFixed(1)} FPS · ${detection.latency_ms} ms model${loopTiming}`
