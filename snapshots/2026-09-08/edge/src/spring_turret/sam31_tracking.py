...
def configure_source(bundle: Path):
    root = bundle.resolve()
    ...
    class Loader(importlib.machinery.SourceFileLoader):
        def get_code(self, fullname):
            ...
            tree = XpuSource().visit(ast.parse(source, self.path))
...
def build_model(torch, checkpoint: Path):
    import sam3
    ...
    return model.eval().requires_grad_(False).to(torch.device("xpu", torch.xpu.current_device()))
