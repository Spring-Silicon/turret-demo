...
Request transfer uses nonblocking pipe writes with a 30-second deadline and
cancellation checks at most 50 ms apart while backpressured. A worker that stops
reading can no longer bypass the response timeout by blocking its caller inside
`write()`. Partial requests are discarded with the failed worker, never reused.
Broken-pipe/OS errors during stream cleanup do not abort recovery; cleanup is
serialized and forgets a reaped process before a second stop can signal it.
CPU tests exercise stalled readers, cancellation, exact binary/legacy payloads,
and repeated cleanup. This does not claim recovery from every GPU-driver hang.
